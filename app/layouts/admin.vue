...
<template>
  <div class="admin-shell">
    <aside v-if="route.path !== '/admin/login'" class="admin-sidebar">
      <NuxtLink
        class="admin-brand admin-logo"
        to="/admin"
        aria-label="Webonova admin"
      >
      ...
      <nav aria-label="Admin navigation">
        <NuxtLink
          ...
          active-class=""
          exact-active-class=""
          :class="{ active: route.hash !== '#inquiries' }"
        >
      ...
      <button class="admin-signout" :disabled="signingOut" @click="signOut">
    ...
    <main
      class="admin-main"
      :class="{ 'login-main': route.path === '/admin/login' }"
    >
      <slot></slot>
    </main>
  </div>
